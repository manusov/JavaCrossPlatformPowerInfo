...
@Override public boolean DynamicalBatteryDetails()
    ...
    if ( (PowerInfo.pal.NativeAgent( null, OPB, 2, OPB_SIZE )) != 0 )
        ...
        if (count!=0)         // Check OPB[0] = Number of devices
            {
//--- changed at v0.75 ---            
//          int i = selected * 128;  // ITEM-DEVICE SELECTION
            int i = selected * 256;
//---            
            int j = 0, k = 0;
            
            //--- OS enumeration, added at v0.75, moved from JComboBox ---
            s = "";
            long b1 = 0;
            char c1 = ' ';
            int m1 = 0;
            ...
            if ( !( ((x==0)|(y==0)) | ((y>100)|(y<36)) ) )
                ...
                //
                // boolean bunits;
                // if ((x&0x40000000)==0) { bunits=false; s1="mWh"; }
                // else { bunits=true; s1="relative ratio units"; }
                //
            ...
            if ( !( ((x==0)|(y==0)) | ((y>100)|(y<16)) ) )
                ...
                //
                // y = -2000000000; // DEBUG UNAVAILABLE PARAMETER REPORTING
                //
                boolean pValid = true;
...
/*        
    ...
    char c=' ';
    int m=0;
    for (int i=0; i<count; i++)         // cycle for devices
        {
        a[i]=" ";
//--- additions at v0.75 ---
        m=0;
//---
        for (int j=0; j<128; j++)       // cycle for qwords in the device names
            {
//--- changes at v0.75 ---
//          b = OPB[ i*128 + m + 8 ];
            b = OPB[ i*256 + m + 8 ];
//---            
            for (int k=0; k<8; k++)     // cycle for bytes in the qword
                {
                c = (char)(b & 0xFF);
                if (c==0) { break; }
                a[i]=a[i]+c;
